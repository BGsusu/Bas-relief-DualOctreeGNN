...
class OcnnAESolver(Solver):
  ...
  def model_forward(self, batch):
    ...
    octree = batch['octree_gt'].cuda()
    model_out = self.model(octree, False)
  ...
  #   # forward the model
  #   output = self.model.forward(batch['octree_gt'].cuda())
  ...
  def eval_step(self, batch):
    # forward the model
    # here 'octree_gt' for bas-relief
    octree_in = batch['octree_gt'].cuda(non_blocking=True)
