...
class BasReliefOUNet(torch.nn.Module):

  def __init__(self, depth, channel_in, nout, full_depth=2, depth_out=6,
               resblk_type='bottleneck', bottleneck=4, feature: str = 'ND'):
    super().__init__()
    self.depth = depth
...
#  def _get_input_feature(self, octree: ocnn.octree.Octree):
...
#    return out
  ...
  def octree_encoder(self, octree, doctree):
    ...
    # data = self._get_input_feature(octree)
